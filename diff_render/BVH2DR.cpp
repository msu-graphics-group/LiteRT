...
//  =========================================================  */
  ...
  CRT_HitDR BVHDR::RayQuery_NearestHitWithGradDShape(float4 posAndNear, float4 dirAndFar, float relax_eps)
  {
    bool stopOnFirstHit = (dirAndFar.w <= 0.0f);
    if(stopOnFirstHit)
      dirAndFar.w *= -1.0f;
...

    {
      ...
      do
      {
        ...
        if(isLeafAndIntersect(travFlags)) 
        {
          ...
          BVH2TraverseF32WithGradDShape(ray_pos, ray_dir, posAndNear.w, instId, geomId, stopOnFirstHit, relax_eps, &hit);
        }
      } while (nodeIdx < 0xFFFFFFFE && !(stopOnFirstHit && hit.primId != uint32_t(-1))); //
    }
  ...
  void BVHDR::BVH2TraverseF32WithGradDShape(const float3 ray_pos, const float3 ray_dir, float tNear,
                                           uint32_t instId, uint32_t geomId, bool stopOnFirstHit,
                                           float relax_eps, /* new */
                                           CRT_HitDR *pHit)
  {
    ...
    while (top >= 0 && !(stopOnFirstHit && pHit->primId != uint32_t(-1)))
    {
      ...
      if (top >= 0 && leftNodeOffset != 0xFFFFFFFF)
      {
        const uint32_t start = EXTRACT_START(leftNodeOffset);
        const uint32_t count = EXTRACT_COUNT(leftNodeOffset);
        IntersectAllPrimitivesInLeafWithGradDShape(ray_pos, ray_dir, tNear, instId, geomId, start, count, relax_eps, pHit);
      }
  ...
  void BVHDR::IntersectAllPrimitivesInLeafWithGradDShape(const float3 ray_pos, const float3 ray_dir,
                                                         float tNear, uint32_t instId, uint32_t geomId,
                                                         uint32_t a_start, uint32_t a_count,
                                                         float relax_eps, /* new */
                                                         CRT_HitDR *pHit)
  {
    ...
    case TYPE_MESH_TRIANGLE:
      ...
      break;
    case TYPE_SDF_SBS_COL:
      OctreeBrickIntersectWithGradDShape(type, ray_pos, ray_dir, tNearSdf, instId, geomId, a_start, a_count, relax_eps, pHit);
      break;
    default:
      break;
    }
  ...
  void BVHDR::OctreeBrickIntersectWithGradDShape(uint32_t type, const float3 ray_pos, const float3 ray_dir,
                                                 float tNear, uint32_t instId, uint32_t geomId,
                                                 uint32_t bvhNodeId, uint32_t a_count,
                                                 float relax_eps, /* new */
                                                 CRT_HitDR *pHit)
  {
    ...
    std::vector<float2> relax_points;
    ...
    while (brick_fNearFar.x < brick_fNearFar.y && pHit->t == old_t)
    {
      ...
      if (tNear < fNearFar.x && vmin <= 0.0f)
      {
        ...
        float2 t_sdf_pair = {qFar, 0.f};

        float t = IntersectDShape(ray_dir, values, d, 0.0f, qFar, start_q, relax_eps, t_sdf_pair);

        if (t_sdf_pair.x <= qFar)
          relax_points.push_back(t_sdf_pair);
    ...
    // TODO: Among relax_points find one before intersection point with lowest SDF (.y)
  }


  float BVHDR::IntersectDShape(const float3 ray_dir, float values[8], float d, 
                               float qNear, float qFar, float3 start_q, float relax_eps, float2 &t_sdf_pair)
  {
        ...
        //    m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_IT)
    {
      ...
      // Looking for SDF min - first or second root of (3*c3)*t^2 + (2*c2)*t + c1 = 0 (if x1 != x2)
      {
        ...
        else if (b_d > EPS)// (2*c2)*t + c1 = 0, ALSO there is no need to check if (2*c2) < 0 - max point, not needed
        {
          t_min = -c1 / b_d;
        }
        ...
        if (t_min > qNear && t_min <= qFar)
        {
          ...
          if (sdf_min >= 0.f && sdf_min < relax_eps) // Found relaxation point
            t_sdf_pair = { t_min, sdf_min };
        }
      }
