...
void litert_test_29_diff_render_forward_pass()
{
  //create renderers for SDF scene and mesh scene
  auto mesh = cmesh4::LoadMeshFromVSGF((scenes_folder_path + "scenes/01_simple_scenes/data/bunny.vsgf").c_str());
  cmesh4::rescale_mesh(mesh, float3(-0.95, -0.95, -0.95), float3(0.95, 0.95, 0.95));
  ...
  LiteImage::SaveImage<float4>("saves/test_29_mesh.bmp", image_mesh); 
  LiteImage::SaveImage<float4>("saves/test_29_sbs.bmp", image_SBS);
  LiteImage::SaveImage<float4>("saves/test_29_sbs_dr.bmp", image_SBS_dr);

  //float psnr_1 = image_metrics::PSNR(image_mesh, image_SBS);
  float psnr_2 = image_metrics::PSNR(image_SBS, image_SBS_dr);

  printf("TEST 29. Differentiable render forward pass\n");

  printf(" 29.1. %-64s", "Diff render of SBS match regular render");
  if (psnr_2 >= 40)
    printf("passed    (%.2f)\n", psnr_2);
  else
    printf("FAILED, psnr = %f\n", psnr_2);
...
void litert_test_30_diff_render_optimize_color()
{
  //create renderers for SDF scene and mesh scene
  auto mesh = cmesh4::LoadMeshFromVSGF((scenes_folder_path + "scenes/01_simple_scenes/data/bunny.vsgf").c_str());
  cmesh4::rescale_mesh(mesh, float3(-0.95, -0.95, -0.95), float3(0.95, 0.95, 0.95));
  ...
  LiteImage::SaveImage<float4>("saves/test_30_mesh.bmp", image_mesh); 
  LiteImage::SaveImage<float4>("saves/test_30_sbs.bmp", image_SBS);
  LiteImage::SaveImage<float4>("saves/test_30_sbs_dr.bmp", image_SBS_dr);

  //float psnr_1 = image_metrics::PSNR(image_mesh, image_SBS);
  float psnr_2 = image_metrics::PSNR(image_mesh, image_SBS_dr);

  printf("TEST 30. Differentiable render optimize color\n");

  printf(" 30.1. %-64s", "Diff render for color reconstruction");
  if (psnr_2 >= 30)
    printf("passed    (%.2f)\n", psnr_2);
  else
    printf("FAILED, psnr = %f\n", psnr_2);
...
void perform_tests_litert(const std::vector<int> &test_ids)
{
  ...
  std::vector<std::function<void(void)>> test_functions = {
      ...
      litert_test_28_enzyme_ad, litert_test_29_diff_render_forward_pass, litert_test_30_diff_render_optimize_color};
