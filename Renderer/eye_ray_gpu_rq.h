...
class MultiRenderer_gpu_rq : public MultiRenderer
{
public:

  MultiRenderer_gpu_rq(uint32_t maxPrimitives) : MultiRenderer(maxPrimitives)
  {
    if(m_pAccelStruct == nullptr)
      m_pAccelStruct = std::make_shared<BVHRT>();
  }
  const char* Name() const override { return "MultiRenderer_gpu_rq";}
...
protected:
  ...
  struct MembersDataGPU
  {
    ...
    VkBuffer AbstractObjectRemapTableBuffer = VK_NULL_HANDLE;
    VkBuffer AbstractObjectGeomTagsBuffer   = VK_NULL_HANDLE;
  } m_vdata;
  ...
  struct AllBufferReferences 
  {
    ...
    VkDeviceAddress AbstractObjectRemapAddr;
    VkDeviceAddress AbstractObjectGeomTags;
  };
  ...
  MultiRenderer_gpu_rq_UBO_Data m_uboData;
