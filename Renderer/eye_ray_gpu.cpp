...
void MultiRenderer_GPU::UpdateTextureMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  ...
  if (vkBeginCommandBuffer(cmdBuff, &beginInfo) != VK_SUCCESS)
    throw std::runtime_error("MultiRenderer_GPU::UpdateTextureMembers: failed to begin command buffer!");
...
void MultiRenderer_GPU::CastRayFloatSingleMegaCmd(uint32_t tidX, float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_GPU::CastRayFloatSingleCmd(VkCommandBuffer a_commandBuffer, uint32_t tidX, float4* out_color)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
...
void MultiRenderer_GPU::CastRayFloatSingleBlock(uint32_t tidX, float4* out_color, uint32_t a_numPasses)
{
  // (1) get global Vulkan context objects
  //
  VkInstance       instance       = m_ctx.instance;
...
void MultiRenderer_GPU::ResetTimeStampMeasurements()
{
  m_tsIdToKernelName.resize(m_timestampPoolSize/2);
  m_kernelTimings.clear();
}

void MultiRenderer_GPU::AccumTimeStampMeasurements(uint32_t a_start, uint32_t a_size)
{
