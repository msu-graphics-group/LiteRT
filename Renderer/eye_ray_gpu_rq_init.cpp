...
{
  auto pObj = std::make_shared<MultiRenderer_gpu_rq>(maxPrimitives);
  pObj->SetVulkanContext(a_ctx);
  pObj->InitVulkanObjects(a_ctx.device, a_ctx.physicalDevice, a_maxThreadsGenerated);
  return pObj;
}

void MultiRenderer_gpu_rq::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
  ...
  auto m_pAccelStructOld = m_pAccelStruct; // save user implementation
  ...
  m_pAccelStruct = std::make_shared<RTX_Proxy>(m_pAccelStructOld, m_pAccelStruct); // wrap both user and RTX implementation with proxy object 
}
...
VkBufferUsageFlags MultiRenderer_gpu_rq::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t MultiRenderer_gpu_rq::GetDefaultMaxTextures() const { return 256; }

void MultiRenderer_gpu_rq::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void MultiRenderer_gpu_rq::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
MultiRenderer_gpu_rq::~MultiRenderer_gpu_rq()
{
...
void MultiRenderer_gpu_rq::InitHelpers()
{
  vkGetPhysicalDeviceProperties(physicalDevice, &m_devProps);
}

VkBufferMemoryBarrier MultiRenderer_gpu_rq::BarrierForObjCounters(VkBuffer a_buffer)
{
...
void MultiRenderer_gpu_rq::InitKernel_PackXYMega(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_gpu_rq/PackXYMega.comp.spv");
...
void MultiRenderer_gpu_rq::InitKernel_CastRayFloatSingleMega(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_gpu_rq/CastRayFloatSingleMega.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_gpu_rq::InitKernel_CastRaySingleMega(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_gpu_rq/CastRaySingleMega.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_gpu_rq::InitKernels(const char* a_filePath)
{
...
void MultiRenderer_gpu_rq::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void MultiRenderer_gpu_rq::ReserveEmptyVectors()
{
...
void MultiRenderer_gpu_rq::InitMemberBuffers()
{
  ...
  }
  {
    ...
    {
      m_vdata.AbstractObjectRemapTableBuffer = vk_utils::createBuffer(device, tablePtrs.tableSize*sizeof(LiteMath::uint2), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
      m_vdata.AbstractObjectGeomTagsBuffer   = vk_utils::createBuffer(device, tablePtrs.geomSize*sizeof(LiteMath::uint), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
      memberVectorsWithDevAddr.push_back(m_vdata.AbstractObjectRemapTableBuffer);
      memberVectorsWithDevAddr.push_back(m_vdata.AbstractObjectGeomTagsBuffer);
    }
    else
    {
      m_vdata.AbstractObjectRemapTableBuffer = VK_NULL_HANDLE;
      m_vdata.AbstractObjectGeomTagsBuffer = VK_NULL_HANDLE;
    }
  }
    ...
    AllocAndBind(memberVectorsWithDevAddr, VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT);
  {
    ...
    if(m_vdata.AbstractObjectRemapTableBuffer != VK_NULL_HANDLE)
      all_references[0].AbstractObjectRemapAddr = vk_rt_utils::getBufferDeviceAddress(device, m_vdata.AbstractObjectRemapTableBuffer);
    else
      all_references[0].AbstractObjectRemapAddr = VkDeviceAddress(0);
    if(m_vdata.AbstractObjectGeomTagsBuffer != VK_NULL_HANDLE)
      all_references[0].AbstractObjectGeomTags = vk_rt_utils::getBufferDeviceAddress(device, m_vdata.AbstractObjectGeomTagsBuffer);
    else
      all_references[0].AbstractObjectGeomTags = VkDeviceAddress(0);
  }
...
VkImage MultiRenderer_gpu_rq::CreateTexture2D(const int a_width, const int a_height, VkFormat a_format, VkImageUsageFlags a_usage)
{
...
VkSampler MultiRenderer_gpu_rq::CreateSampler(const Sampler& a_sampler) // TODO: implement this function correctly
{
...
VkImageView MultiRenderer_gpu_rq::CreateView(VkFormat a_format, VkImage a_image)
{
...
void MultiRenderer_gpu_rq::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[MultiRenderer_gpu_rq::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
MultiRenderer_gpu_rq::MemLoc MultiRenderer_gpu_rq::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
MultiRenderer_gpu_rq::MemLoc MultiRenderer_gpu_rq::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "MultiRenderer_gpu_rq::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void MultiRenderer_gpu_rq::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void MultiRenderer_gpu_rq::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 MultiRenderer_gpu_rq::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
MultiRenderer_gpu_rq::MegaKernelIsEnabled MultiRenderer_gpu_rq::m_megaKernelFlags;
