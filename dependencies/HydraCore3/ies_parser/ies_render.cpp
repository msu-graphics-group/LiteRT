...
std::vector<float> CreateSphericalTextureFromIES(const std::string& a_iesData, int* pW, int* pH)
{
  ...
  //
  //
  if (std::abs(verticalStart) < eps && std::abs(verticalEnd - 90.0f) < eps) // 0-90
    h = iesOldCrap.photo.num_vert_angles * 2;
  else if (std::abs(verticalStart - 90.0f) < eps && std::abs(verticalEnd - 180.0f) < eps) // 90-180
    h = iesOldCrap.photo.num_vert_angles * 2;
  else // 0-180
  ...
  //
  if (std::abs(horizontStart) < eps && std::abs(horizontEnd) < eps) // 0-0
    w = 1;
  else if (std::abs(horizontStart) < eps && std::abs(horizontEnd - 90.0f) < eps) // 0-90, If it is 90�, the distribution is symmetric in each quadrant.
  {
    w = iesOldCrap.photo.num_horz_angles * 4;
    reflectType = REFLECT4;
  }
  else if (std::abs(horizontStart) < eps && std::abs(horizontEnd - 180.0f) < eps) // 0-180,  If it is 180�, the distribution is symmetric about a vertical plane.
  {
    w = iesOldCrap.photo.num_horz_angles * 4;
    reflectType = REFLECT4;
  }
  else if (std::abs(horizontStart - 90.0f) < eps && std::abs(horizontEnd - 180.0f) < eps) // 90-180
  {
    w = iesOldCrap.photo.num_horz_angles * 2;
    reflectType = REFLECT2;
  }
  else // if (std::abs(horizontStart) < eps && std::abs(horizontEnd - 360.0f) < eps) // 0-360
  ...
  if (reflectType == REFLECT4)
  {
    for (float thetaGrad = verticalStart; (thetaGrad <= verticalEnd); thetaGrad += stepTheta)
    {
      ...
      for (float phiGrad = 0.0f; (phiGrad <= 90.0f); phiGrad += stepPhi)
      {
        ...
        int iX1 = int((phi1*0.5f*INV_PI)*float(w) + 0.5f);
