...
public:
  ...
  Vector2D(size_t rows, size_t cols, const T& value = T{})
      : n_(rows), m_(cols), data_(rows*cols, value) {}
public:
  T& operator[](std::pair<size_t, size_t> idx) { return data_[n_*idx.first+idx.second]; }
  const T& operator[](std::pair<size_t, size_t> idx) const { return data_[n_*idx.first+idx.second]; }
public:
...
uint32_t weights_offset(NURBSHeader h) { 
  return (h.n + 1)*(h.m + 1)*sizeof(LiteMath::float4); 
}
inline
uint32_t u_knots_offset(NURBSHeader h) { 
  return weights_offset(h) + (h.n + 1)*(h.m + 1)*sizeof(float); 
}
inline
uint32_t v_knots_offset(NURBSHeader h) {
  return u_knots_offset(h) + (h.n + h.p + 2)*sizeof(float);
}
...
public:
  DecomposedNURBS(const float *data, NURBSHeader h)
      : points(reinterpret_cast<const LiteMath::float4*>(data)),
        ...
        header(h) {}
public:
  LiteMath::float4 point(uint i, uint j) const { return points[i*(header.n+1) + j]; }
  float w(uint i, uint j) const { return weights[i*(header.n+1) + j]; }
