...
void addToVector(std::vector<float>& v, float data[16]) {
  v.insert(v.end(), data, data + 16);
}
      ...
      for (size_t x = 0; x < grid.size - 1; x++)
      {
        ...
        if (getDensity(node) > 0.0f) {
          auto addCell = [&](uint3 coords) {
            addToVector(sparseGrid, &grid.data[16 * (coords[0] + coords[1] * grid.size + coords[2] * grid.size * grid.size)]);
          };
          ...
          auto addPointer = [&](uint3 coords) {
            ...
            if (coordsToIdx.find(spaceCoords) == coordsToIdx.end()) {
              coordsToIdx[spaceCoords] = sparseGrid.size() / 16;
              addToVector(sparseGrid, &grid.data[16 * (coords[0] + coords[1] * grid.size + coords[2] * grid.size * grid.size)]);
            }
