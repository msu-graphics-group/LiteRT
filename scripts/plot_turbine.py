import sys
if len(sys.argv) == 1:
    print('Usage: python3 plot_turbine.py input_file.csv [save-format]')
...
df = df[(df.model_name == 'Turbine-obj') & (df['type'] != 'MESH')]
...
def normal_plot():
    ...
    ax.set_title('Зависимость PSNR от размера модели')
    ax.legend(title='Тип модели', loc='lower right')
    ax.set_xlabel('Размер модели (МБ)')
    ax.set_ylabel('Среднее PSNR')
